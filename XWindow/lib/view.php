...
class view {
	...
	private $_view_path = VIEW_PATH;
	static $_pos = null;
    static $_section = null;

	static function load($file, $set = null, $view_path = VIEW_PATH) {
		if (is_int(strripos($file, '..'))) {
			die("error view file name:$file");
		}
		$file = str_replace('.', '/', $file);

		return new view($file, $set, $view_path);
	}

	public function __construct($file, $set = null, $view_path = VIEW_PATH) {
		if (isset(self::$_pos)) {
	...
	public function __toString() {
		...
		$this->_view['file'] = $this->_view_path . $this->_view['file'] . '.php';
